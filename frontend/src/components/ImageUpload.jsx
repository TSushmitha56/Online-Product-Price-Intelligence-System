...
export default function ImageUpload({ onProductRecognized }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/upload-image/', { method: 'POST', body: formData });
      ...
      if (response.ok) {
        ...
        setUploadStatus({ type: 'error', message: data.message || 'Failed to upload image' });
      }
    } catch (error) {
  ...
  const handleRecognize = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUploadStatus({ type: 'error', message: data.message || 'Failed to recognize product' });
      }
    } catch (error) {
      setUploadStatus({ type: 'error', message: `Recognition failed: ${error.message}` });
