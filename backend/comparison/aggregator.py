...
def aggregate_prices(
    ...
    for p in filtered_products:
        title = p.get('title', '')
        # Accept if title fuzzy matches the product query (lowered threshold for better recall)
        if is_match(normalized_query, normalize_name(title), threshold=55.0):
    ...
    # 5. Calculate statistics — only for items that have a price
    prices = [p['price'] for p in matched_group if p.get('price') is not None and p['price'] > 0]
    
    # If no items have a price at all, still return them so user sees results
    # (e.g. Amazon showing "See options" without a listed price)
    if not prices:
        scored_offers = []
        for p in matched_group:
            p['comparison_score'] = 0
            ...
            "matched_products": [
                {
                    ...
                    "price_stats": {"lowest": None, "highest": None, "average": None, "count": 0},
    ...
    for p in matched_group:
        if p.get('price') and p['price'] > 0:
            ...
            score = 0  # Items with no price get lowest score so they show last
